'use client'

import { useState, useRef, type KeyboardEvent } from 'react'
import { Button } from '@/components/ui/button'
import { SpinnerIcon } from '@/components/ui/icons'
import type { MoodLevel, WellnessSymptom, ParentType } from '@/types'

type Step = 'chat' | 'details' | 'journal' | 'response'

const SYMPTOMS: { value: WellnessSymptom; label: string }[] = [
  { value: 'anxious',      label: 'Anxious'      },
  { value: 'exhausted',    label: 'Exhausted'    },
  { value: 'overwhelmed',  label: 'Overwhelmed'  },
  { value: 'lonely',       label: 'Lonely'       },
  { value: 'tearful',      label: 'Tearful'      },
  { value: 'irritable',    label: 'Irritable'    },
  { value: 'disconnected', label: 'Disconnected' },
  { value: 'hopeful',      label: 'Hopeful'      },
  { value: 'supported',    label: 'Supported'    },
  { value: 'energized',    label: 'Energized'    },
]

function scoreToMoodLevel(score: number): MoodLevel {
  const map: Record<number, MoodLevel> = {
    1: 'very_low', 2: 'low', 3: 'neutral', 4: 'good', 5: 'great',
  }
  return map[Math.min(5, Math.max(1, Math.round(score)))]
}

function getOpeningMessage(parentType: ParentType | null, firstName: string): string {
  if (parentType === 'mom')
    return "Hey mama. Before we check in on the little one \u2014 how are YOU feeling today? Tell me as much or as little as you\u2019d like."
  if (parentType === 'dad')
    return "Hey papa. Before we check in on the little one \u2014 how are YOU feeling today? Tell me as much or as little as you\u2019d like."
  const name = firstName?.split(' ')[0] || 'you'
  return `Hey ${name}. Before we check in on the little one \u2014 how are YOU feeling today? Tell me as much or as little as you\u2019d like.`
}

interface WellnessCheckinProps {
  parentType: ParentType | null
  firstName: string
}

export function WellnessCheckin({ parentType, firstName }: WellnessCheckinProps) {
  const [step, setStep] = useState<Step>('chat')

  // Step 1 — conversational chat
  const [chatInput, setChatInput]     = useState('')
  const [chatUserMsg, setChatUserMsg] = useState('')
  const [nestReply, setNestReply]     = useState<string | null>(null)
  const [chatLoading, setChatLoading] = useState(false)
  const [chatError, setChatError]     = useState<string | null>(null)
  const textareaRef = useRef<HTMLTextAreaElement>(null)

  // Silent mood tracking — never shown to user
  const [moodScore, setMoodScore] = useState<number>(3)

  // Steps 2 & 3
  const [energy, setEnergy]         = useState<number>(0)
  const [sleepHours, setSleepHours] = useState('')
  const [symptoms, setSymptoms]     = useState<WellnessSymptom[]>([])
  const [journal, setJournal]       = useState('')

  // Step 4
  const [aiResponse, setAiResponse]   = useState('')
  const [submitting, setSubmitting]   = useState(false)
  const [submitError, setSubmitError] = useState<string | null>(null)

  const openingMessage = getOpeningMessage(parentType, firstName)

  function handleTextareaResize() {
    const el = textareaRef.current
    if (!el) return
    el.style.height = 'auto'
    el.style.height = Math.min(el.scrollHeight, 160) + 'px'
  }

  function handleChatKeyDown(e: KeyboardEvent<HTMLTextAreaElement>) {
    if (e.key === 'Enter' && !e.shiftKey) {
      e.preventDefault()
      sendChat()
    }
  }

  async function sendChat() {
    const trimmed = chatInput.trim()
    if (!trimmed || chatLoading || chatUserMsg) return
    setChatUserMsg(trimmed)
    setChatInput('')
    setChatLoading(true)
    setChatError(null)
    try {
      const res = await fetch('/api/nest/mood', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ message: trimmed, parent_type: parentType, first_name: firstName }),
      })
      if (!res.ok) throw new Error('API error')
      const data = await res.json()
      setNestReply(data.nest_response)
      setMoodScore(data.mood_score ?? 3)
    } catch {
      const name = parentType === 'mom' ? 'mama' : parentType === 'dad' ? 'papa' : (firstName?.split(' ')[0] || 'you')
      setNestReply(`Thank you for sharing that with me, ${name}. I\u2019m right here with you.`)
      setChatError('(Nest had a moment \u2014 continue below)')
    } finally {
      setChatLoading(false)
    }
  }

  function toggleSymptom(s: WellnessSymptom) {
    setSymptoms(prev => prev.includes(s) ? prev.filter(x => x !== s) : [...prev, s])
  }

  async function handleSubmit() {
    if (!energy) return
    setSubmitting(true)
    setSubmitError(null)
    try {
      const res = await fetch('/api/wellness/checkin', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({
          mood:         scoreToMoodLevel(moodScore),
          energy_level: energy,
          sleep_hours:  sleepHours ? parseFloat(sleepHours) : null,
          symptoms,
          journal_text: journal || null,
          checkin_date: new Date().toISOString().split('T')[0],
        }),
      })
      if (!res.ok) throw new Error('failed')
      const data = await res.json()
      setAiResponse(data.ai_response)
      setStep('response')
    } catch {
      setSubmitError('Something went wrong. Please try again.')
    } finally {
      setSubmitting(false)
    }
  }

  function reset() {
    setStep('chat')
    setChatInput('')
    setChatUserMsg('')
    setNestReply(null)
    setChatLoading(false)
    setChatError(null)
    setMoodScore(3)
    setEnergy(0)
    setSleepHours('')
    setSymptoms([])
    setJournal('')
    setAiResponse('')
    setSubmitError(null)
  }

  // ─────────────────────────────────────────────────────────────────────
  // Step: Chat
  // ─────────────────────────────────────────────────────────────────────
  if (step === 'chat') {
    return (
      <div className="bg-white rounded-2xl border border-sage-200 p-6">
        {/* Nest opening message */}
        <div className="flex items-end gap-2.5 mb-5">
          <div className="w-7 h-7 rounded-full bg-brand-100 flex items-center justify-center flex-shrink-0">
            <span className="font-display italic text-brand-700 text-[11px]">N</span>
          </div>
          <div className="bg-warm-100 border border-warm-400 rounded-2xl rounded-bl-sm px-5 py-3.5 text-sm text-brand-900 leading-relaxed">
            {openingMessage}
          </div>
        </div>

        {/* User's sent message */}
        {chatUserMsg && (
          <div className="flex justify-end mb-5">
            <div className="max-w-[80%] bg-brand-600 text-white rounded-2xl rounded-tr-sm px-5 py-3.5 text-sm leading-relaxed">
              {chatUserMsg}
            </div>
          </div>
        )}

        {/* Typing indicator */}
        {chatLoading && (
          <div className="flex items-end gap-2.5 mb-5">
            <div className="w-7 h-7 rounded-full bg-brand-100 flex items-center justify-center flex-shrink-0">
              <span className="font-display italic text-brand-700 text-[11px]">N</span>
            </div>
            <div className="bg-warm-100 border border-warm-400 rounded-2xl rounded-bl-sm px-4 py-3">
              <div className="flex gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-brand-400 animate-bounce [animation-delay:0ms]" />
                <span className="w-1.5 h-1.5 rounded-full bg-brand-400 animate-bounce [animation-delay:160ms]" />
                <span className="w-1.5 h-1.5 rounded-full bg-brand-400 animate-bounce [animation-delay:320ms]" />
              </div>
            </div>
          </div>
        )}

        {/* Nest's reply */}
        {nestReply && !chatLoading && (
          <div className="flex items-end gap-2.5 mb-5">
            <div className="w-7 h-7 rounded-full bg-brand-100 flex items-center justify-center flex-shrink-0">
              <span className="font-display italic text-brand-700 text-[11px]">N</span>
            </div>
            <div className="bg-warm-100 border border-warm-400 rounded-2xl rounded-bl-sm px-5 py-3.5 text-sm text-brand-900 leading-relaxed">
              {nestReply}
            </div>
          </div>
        )}

        {/* Text input — visible only before sending */}
        {!chatUserMsg && (
          <div className="flex items-end gap-3 mt-2">
            <textarea
              ref={textareaRef}
              value={chatInput}
              onChange={e => setChatInput(e.target.value)}
              onKeyDown={handleChatKeyDown}
              onInput={handleTextareaResize}
              placeholder="I\u2019m feeling\u2026"
              rows={3}
              disabled={chatLoading}
              className="
                flex-1 resize-none bg-sage-50 border border-sage-200
                rounded-xl px-4 py-3 text-sm text-brand-900
                placeholder-sage-400 leading-relaxed
                focus:outline-none focus:ring-2 focus:ring-brand-200 focus:border-transparent
                disabled:opacity-50 transition-all
              "
            />
            <button
              onClick={sendChat}
              disabled={!chatInput.trim() || chatLoading}
              className="
                w-10 h-10 rounded-full bg-brand-600 text-white flex-shrink-0
                flex items-center justify-center transition-colors
                hover:bg-brand-700 disabled:bg-sage-200 disabled:cursor-not-allowed
              "
            >
              {chatLoading
                ? <SpinnerIcon size={15} className="text-white" />
                : <span className="text-base leading-none">↑</span>
              }
            </button>
          </div>
        )}

        {chatError && (
          <p className="text-[11px] text-sage-400 italic mt-2">{chatError}</p>
        )}

        {nestReply && !chatLoading && (
          <button
            onClick={() => setStep('details')}
            className="
              w-full mt-5 py-3 rounded-xl bg-brand-600 text-white
              text-[11px] uppercase tracking-[0.2em]
              hover:bg-brand-700 transition-colors
            "
          >
            Continue
          </button>
        )}
      </div>
    )
  }

  // ─────────────────────────────────────────────────────────────────────
  // Step: Details
  // ─────────────────────────────────────────────────────────────────────
  if (step === 'details') {
    return (
      <div className="bg-white rounded-2xl border border-sage-200 p-6">
        <h2 className="font-display text-xl italic text-brand-900 mb-7">
          A little more about today
        </h2>

        {/* Energy */}
        <div className="mb-7">
          <p className="text-[10px] uppercase tracking-[0.18em] text-sage-400 mb-3">Energy level</p>
          <div className="flex gap-2 items-center flex-wrap">
            {[1, 2, 3, 4, 5].map(n => (
              <button
                key={n}
                onClick={() => setEnergy(n)}
                className={`
                  w-10 h-10 rounded-full text-sm border transition-all
                  ${
                    energy === n
                      ? 'border-brand-600 bg-brand-600 text-white font-medium'
                      : 'border-sage-200 text-sage-500 hover:border-sage-400'
                  }
                `}
              >
                {n}
              </button>
            ))}
            <span className="text-[11px] text-sage-400 ml-1">1 low – 5 high</span>
          </div>
        </div>

        {/* Sleep */}
        <div className="mb-7">
          <p className="text-[10px] uppercase tracking-[0.18em] text-sage-400 mb-2">
            Hours of sleep{' '}
            <span className="normal-case tracking-normal text-sage-300">(optional)</span>
          </p>
          <input
            type="number"
            min="0" max="24" step="0.5"
            placeholder="e.g. 5.5"
            value={sleepHours}
            onChange={e => setSleepHours(e.target.value)}
            className="
              w-28 rounded-xl border border-sage-200 bg-sage-50
              px-3 py-2 text-sm text-brand-900
              focus:outline-none focus:ring-2 focus:ring-brand-200
            "
          />
        </div>

        {/* Symptoms */}
        <div className="mb-8">
          <p className="text-[10px] uppercase tracking-[0.18em] text-sage-400 mb-3">
            Anything resonating today?{' '}
            <span className="normal-case tracking-normal text-sage-300">(select all that apply)</span>
          </p>
          <div className="grid grid-cols-2 gap-2">
            {SYMPTOMS.map(s => (
              <button
                key={s.value}
                onClick={() => toggleSymptom(s.value)}
                className={`
                  text-left px-4 py-2.5 rounded-xl text-sm border transition-all
                  ${
                    symptoms.includes(s.value)
                      ? 'border-brand-500 bg-brand-50 text-brand-700 font-medium'
                      : 'border-sage-200 text-sage-500 hover:border-sage-300'
                  }
                `}
              >
                {s.label}
              </button>
            ))}
          </div>
        </div>

        <div className="flex gap-3">
          <button
            onClick={() => setStep('chat')}
            className="flex-1 py-3 rounded-xl border border-sage-200 text-[11px] uppercase tracking-[0.18em] text-sage-500 hover:bg-sage-50 transition-colors"
          >
            Back
          </button>
          <button
            onClick={() => setStep('journal')}
            disabled={!energy}
            className="flex-1 py-3 rounded-xl bg-brand-600 text-white text-[11px] uppercase tracking-[0.2em] hover:bg-brand-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
            Continue
          </button>
        </div>
      </div>
    )
  }

  // ─────────────────────────────────────────────────────────────────────
  // Step: Journal
  // ─────────────────────────────────────────────────────────────────────
  if (step === 'journal') {
    return (
      <div className="bg-white rounded-2xl border border-sage-200 p-6">
        <h2 className="font-display text-xl italic text-brand-900 mb-1">
          Anything else on your mind?
        </h2>
        <p className="text-sm text-sage-400 mb-5">This is just for you. Write freely.</p>
        <textarea
          value={journal}
          onChange={e => setJournal(e.target.value)}
          placeholder="Whatever you want to get out\u2026"
          rows={5}
          className="
            w-full resize-none bg-sage-50 border border-sage-200
            rounded-xl px-4 py-3 text-sm text-brand-900
            placeholder-sage-400 leading-relaxed
            focus:outline-none focus:ring-2 focus:ring-brand-200 mb-6
          "
        />
        {submitError && (
          <p className="text-sm text-red-400 mb-4">{submitError}</p>
        )}
        <div className="flex gap-3">
          <button
            onClick={() => setStep('details')}
            className="flex-1 py-3 rounded-xl border border-sage-200 text-[11px] uppercase tracking-[0.18em] text-sage-500 hover:bg-sage-50 transition-colors"
          >
            Back
          </button>
          <button
            onClick={handleSubmit}
            disabled={submitting}
            className="flex-1 py-3 rounded-xl bg-brand-600 text-white text-[11px] uppercase tracking-[0.2em] hover:bg-brand-700 transition-colors disabled:opacity-60 flex items-center justify-center gap-2"
          >
            {submitting && <SpinnerIcon size={13} />}
            Send to Nest
          </button>
        </div>
      </div>
    )
  }

  // ─────────────────────────────────────────────────────────────────────
  // Step: Response
  // ─────────────────────────────────────────────────────────────────────
  return (
    <div className="bg-white rounded-2xl border border-sage-200 p-6">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-10 h-10 rounded-full bg-brand-100 flex items-center justify-center flex-shrink-0">
          <span className="font-display italic text-brand-700 text-sm">N</span>
        </div>
        <div>
          <p className="font-display italic text-brand-900">Nest</p>
          <p className="text-[10px] uppercase tracking-[0.15em] text-sage-400 mt-0.5">For you</p>
        </div>
      </div>
      <p className="text-sm text-brand-900 leading-relaxed mb-8 whitespace-pre-wrap">
        {aiResponse}
      </p>
      <button
        onClick={reset}
        className="w-full py-3 rounded-xl border border-sage-200 text-[11px] uppercase tracking-[0.18em] text-sage-500 hover:bg-sage-50 transition-colors"
      >
        Done for today
      </button>
    </div>
  )
}
